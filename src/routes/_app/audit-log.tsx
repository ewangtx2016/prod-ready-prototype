...
function Inner() {
  ...
  }, [logs, roleFilter, actionFilter, orgFilter, keyword, dateFrom, dateTo]);
  const hasFilter = roleFilter !== "all" || actionFilter !== "all" || orgFilter !== "all" || !!keyword || !!dateFrom || !!dateTo;
  const reset = () => { setRoleFilter("all"); setActionFilter("all"); setOrgFilter("all"); setKeyword(""); setDateFrom(""); setDateTo(""); };
  ...
  return (
    <div>
      ...
      <DevNote prd="§4.3 §16.5" title="审计日志">
        <div>· 每条日志包含：操作人 / 角色 / 时间 / IP / 类型 / 详情 / 机构</div>
      ...
      <div className="mb-3 grid grid-cols-2 gap-3 rounded-lg border bg-card p-3 md:grid-cols-8">
        ...
        <div className="col-span-2 text-xs text-muted-foreground md:col-span-8">共 {filtered.length} 条</div>
      </div>
      <div className="rounded-lg border bg-card">
        <Table>
          <TableHeader><TableRow><TableHead>时间</TableHead><TableHead>操作人</TableHead><TableHead>角色</TableHead><TableHead>机构</TableHead><TableHead>IP</TableHead><TableHead>类型</TableHead><TableHead>详情</TableHead></TableRow></TableHeader>
          <TableBody>
            {paged.map((l) => (
              <TableRow key={l.id}>
            ...
            {filtered.length === 0 && <TableRow><TableCell colSpan={7} className="py-12 text-center text-muted-foreground">暂无日志</TableCell></TableRow>}
          </TableBody>
        </Table>
        <Pagination />
      </div>
